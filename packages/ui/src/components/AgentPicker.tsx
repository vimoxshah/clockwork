        ...
        <div className="space-y-1" role="listbox" aria-label="Search results">
          ...
          {filtered.slice(0, 6).map((p) => (
            <button
              ...
              className="flex w-full items-center gap-2 rounded-lg border border-border px-3 py-2 text-left text-compact hover:bg-surface-hover"
            >
      ...
      {!term && (
        <>
          ...
          {value && (() => {
            ...
            if (!sel) return null;
            return (
              <div className="flex items-center gap-2 rounded-lg border border-strong bg-surface-active px-3 py-2 text-compact" data-testid="selected-agent">
              ...
              <div className="grid grid-cols-2 gap-2">
                {recommended.map(({ profile: p, intent, glyph }) => (
                  <button
                    ...
                    title={intent}
                    className="rounded-lg border border-border px-3 py-2 text-left text-compact hover:border-strong hover:bg-surface-hover"
                  >
                    <span aria-hidden className="mr-1.5">{glyph}</span>
                    {p.name}
                    <div className="text-xxs text-dim">{intent}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          <button
            ...
            className="text-compact text-muted underline-offset-2 hover:text-fg hover:underline"
            ...
            <div className="max-h-52 space-y-1 overflow-auto rounded-lg border border-border p-2" role="radiogroup" aria-label="All agents">
              {listAll.map((p) => (
                <button
                  ...
                  onClick={() => onChange(p.id)}
                  className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-left text-compact hover:bg-surface-hover ${value === p.id ? 'bg-surface-active' : ''}`}
                >
                  <span aria-hidden>{p.avatar ?? '◆'}</span>
                  {p.name}
                  {p.slug && <span className="mono ml-auto text-xxs text-dim">@{p.slug}</span>}
                </button>
              ))}
            </div>
          )}
