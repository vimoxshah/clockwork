...
import { Folder, FolderOpen, ChevronLeft, Check, GitBranch, FileText } from 'lucide-react';
...
export function FolderBrowserDialog({
  ...
  /**
   * Extension list (e.g. ['ics', 'ical']) — when given, matching files are
   * listed alongside folders and picking a FILE confirms the dialog with its
   * full path. Omit for the original folders-only repo picker; that mode is
   * unchanged.
   */
  files?: string[];
}): JSX.Element {
  ...
  async function load(p: string): Promise<void> {
    ...
    setError(null);
    try {
      const r = await api.browseFs(p, files?.join(','));
      setPath(r.path);
      setParent(r.parent);
      // File-picking mode: keep whatever the daemon already filtered to
      // (dirs + matching files). Folder-picking mode (files absent): show
      // only directories, exactly as before.
      setEntries(files ? r.entries : r.entries.filter((e) => e.type === 'dir'));
    } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogTitle>{files ? 'Choose a file' : 'Choose a repository folder'}</DialogTitle>
        <DialogDescription>
          {files ? 'Scoped to your home directory.' : 'Scoped to your home directory. Git repos are marked.'}
        </DialogDescription>
          ...
          <div className="mt-2 max-h-72 overflow-auto rounded-lg border border-border bg-bg p-1">
            {entries.length === 0 && (
              <p className="p-4 text-center text-xs text-dim">
                {files ? 'No matching files or folders here.' : 'No folders here.'}
              </p>
            )}
            {entries.map((e) => (
              <button
